...
DB.load = function() {
    ...
    alasql('DROP TABLE IF EXISTS preset;');
	alasql('CREATE TABLE preset(id INT IDENTITY, name STRING, tablestring STRING, columnstring STRING, displaystring STRING);');
	var ppre = alasql.promise('SELECT MATRIX * FROM CSV("data/PRESET-PRESET.csv", {headers: true})').then(
			function(tabs) {
				for (var i = 0; i < tabs.length; i++) {
					alasql('INSERT INTO preset VALUES(?,?,?,?,?);', tabs[i]);
				}
			});
    
     //tableset
    alasql('DROP TABLE IF EXISTS tableset;');
	alasql('CREATE TABLE tableset(id INT IDENTITY, name STRING, tablestring STRING, columnstring STRING, displaystring STRING);');
	var ptable = alasql.promise('SELECT MATRIX * FROM CSV("data/TABLESET-TABLESET.csv", {headers: true})').then(
			function(tables) {
				for (var i = 0; i < tables.length; i++) {
					alasql('INSERT INTO tableset VALUES(?,?,?,?,?);', tables[i]);
				}
			});
			
	// personal info
	alasql('DROP TABLE IF EXISTS emp;');
	alasql('CREATE TABLE emp(id INT IDENTITY, number STRING, name STRING, sex INT, birthday DATE, tel STRING, ctct_name STRING, ctct_addr STRING, ctct_tel STRING, pspt_no STRING, pspt_date STRING, pspt_name STRING, rental STRING,nationality STRING);');
	...
	// reload html
	Promise.all([ pemp, paddr, pfamily, pedu, pchoice,ptable,ppre ]).then(function() {
